...
var require_shared = __commonJS({
  "node_modules/nodemailer/lib/shared/index.js"(exports, module) {
    ...
    module.exports._logFunc = (logger4, level, defaults, data, message, ...args) => {
      let entry = {};
      ...
      logger4[level](entry, message, ...args);
    };
    module.exports.getLogger = (options, defaults) => {
      ...
      let logger4 = options.logger;
      if (options.logger === true) {
        logger4 = createDefaultLogger(levels);
      }
      levels.forEach((level) => {
    ...
    function createDefaultLogger(levels) {
      ...
      let logger4 = {};
      levels.forEach((level) => {
        logger4[level] = print.bind(null, level);
      });
      return logger4;
    }
  }
...
var vpnClientService_exports = {};
__export(vpnClientService_exports, {
  createVpnClient: () => createVpnClient,
  getUserVpnClient: () => getUserVpnClient,
  getUserVpnClients: () => getUserVpnClients,
  getUserVpnClientsFromTable: () => getUserVpnClientsFromTable,
  revokeVpnClient: () => revokeVpnClient
});
...
async function ensureUserExists2(userId, supabase2) {
  try {
    const { data: existingUser, error: checkError } = await supabase2.from("users").select("id").eq("id", userId).single();
    if (existingUser) {
      console.log("User already exists:", userId);
      return true;
    }
    console.log("User does not exist, creating placeholder user:", userId);
    const { error: insertError } = await supabase2.from("users").insert({
      id: userId,
...
async function handlePaymentSuccess(userId, productId, amount, currency, stripeSessionId, customerEmail) {
  try {
    ...
    const userExists = await ensureUserExists2(userId, supabase2);
    if (!userExists) {
      throw new Error("Failed to ensure user exists in database");
    }
    console.log("Saving purchase record...");
    const { data: purchase, error: purchaseError } = await supabase2.from("purchases").insert({
      user_id: userId,
      product_id: productId,
      amount,
      currency,
    ...
    const expiryDays = getExpiryDaysForProduct2(productId);
    const now = /* @__PURE__ */ new Date();
    let expiresAt = null;
    if (isVpnProduct) {
    ...
    const { data: accessToken, error: tokenError } = await supabase2.from("access_tokens").insert({
      user_id: userId,
      product_id: productId,
      token,
      ...
      // VPN is immediately activated on payment
    }).select().single();
    if (tokenError) {
...
function getExpiryDaysForProduct2(productId) {
  const productDays = {
    "vpn-3days": 3,
    "vpn-7days": 7,
    "vpn-14days": 14,
...
async function GET(request) {
  console.log("\u{1F514} PAYMENT CALLBACK STARTED");
